...
public class PerfilOperador {
    ...
    public boolean mostrarRobots(AdmRobots admRobot){
        return admRobot.mostrarRobots();
    }
    
    public boolean mostrarCiudadanos(AdmCiudadanos admCiudadanos){
        return admCiudadanos.mostrarCiudadanos();
    }
    ...
    public boolean simulate(AdmCiudadanos admCiudadanos, AdmRobots admRobots, AdmReglas admReglas, AdmDron admDron, AdmCargaficios admCargaficios, AdmAnomalias admAnomalias, CInteligencia cInteligencia, CantTareasUsadas cantTareasUsadas, AdmEdificio admEdificio  ){
        admCiudadanos.pedirTareas(cantTareasUsadas, admReglas.getListaReglas().get(1)); //esta funcion recorre cada ciudadano y mediante un random luego pide tarea a los robots que posea (si es que posee) (Ya tarea y todo eso ya hay procesos de registro)
        
        int valorMinimo = 25;
        if (admReglas.getListaReglas().get(0).getActiva()==true){ valorMinimo=admReglas.getListaReglas().get(0).getValorMinimoBateria(); }
        admAnomalias.activarAnomalias(admEdificio.getListaEdificios());
        ...
        admCargaficios.cargarDispositivosConectados();
        return admRobots.conectarRobots(admCargaficios); //se llama al AdmCargaficios especifico y se conectan los robots, si este posee disponible, es un booleano, si da false es que no se pudieron conectar todos los robots descargados
